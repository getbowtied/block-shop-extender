( function( blocks, components, editor, i18n, element ) {
	...
	/* Register Block */
	registerBlockType( 'getbowtied/bs-posts-grid', {
		title: i18n.__( 'Posts Grid', 'block-shop-extender' ),
		...
		edit: function( props ) {
			...
			function _isLoadingText(){
				if ( attributes.isLoading  === false ) {
					return i18n.__( 'Update', 'block-shop-extender' );
				} else {
					return i18n.__( 'Updating', 'block-shop-extender' );
				}
			}
			...
			function renderResults() {
				...
				if( posts.length > 0) {
					...
					for ( let i = 0; i < posts.length; i++ ) {
						...
						let img = '';
						let img_class = 'gbt_18_bs_editor_posts_grid_noimg';
						if ( posts[i]['fimg_url'] ) { img = posts[i]['fimg_url']; img_class = 'gbt_18_bs_editor_posts_grid_with_img'; } else { img_class = 'gbt_18_bs_editor_posts_grid_noimg'; img = ''; };

						postElements.push(
							el( "div", 
								{
									key: 		'gbt_18_bs_editor_posts_grid_item_' + posts[i].id, 
									className: 	'gbt_18_bs_editor_posts_grid_item'
								},
								el( "a", 
									{
									...
									el( "span", 
										{
											key: 		'gbt_18_bs_editor_posts_grid_title',
			...
			return [
				el(
					...
					el(
						...
						el( 'label', { className: 'components-base-control__label' }, i18n.__( 'Categories:', 'block-shop-extender' ) ),
						el(
							'div',
							{
						...
						el(
							SelectControl,
							{
								key: 'bs-posts-grid-order-by',
								options:
									[
										{ value: 'title_asc',   label: i18n.__( 'Alphabetical Ascending', 'block-shop-extender' ) },
										{ value: 'title_desc',  label: i18n.__( 'Alphabetical Descending', 'block-shop-extender' ) },
										{ value: 'date_asc',   	label: i18n.__( 'Date Ascending', 'block-shop-extender' ) },
										{ value: 'date_desc',  	label: i18n.__( 'Date Descending', 'block-shop-extender' ) },
									],
	              				label: i18n.__( 'Order By', 'block-shop-extender' ),
						...
						el(
							RangeControl,
							{
								...
								min: 1,
								max: 20,
								label: i18n.__( 'Number of Posts', 'block-shop-extender' ),
						...
						el(
							RangeControl,
							{
								key: "bs-posts-grid-columns",
								value: attributes.columns,
								allowReset: false,
								initialPosition: 3,
								min: 1,
								max: 4,
								label: i18n.__( 'Columns', 'block-shop-extender' ),
				...
				el( 'div',
					{
						key: 		'gbt_18_bs_posts_grid',
						className: 	'gbt_18_bs_posts_grid'	
					},
					el(
						'div',
						{
